Public Class Arriendo
    ...
    Public Sub GenerarDayPanel(ByVal dias As Integer)
    ...
    Private Sub showAppointmentDetail(sender As Object, e As EventArgs)
        ...
        Dim appID1 As Integer = CType(sender, LinkLabel).Tag
        Dim sql As String = "SELECT Arriendos.ID,Arriendos.Fecha,Arriendos.IdProducto,Arriendos.Letrabloque,Arriendos.NombreCliente,Productos.NombreProducto,Arriendos.Comentario From Arriendos INNER JOIN Productos ON Arriendos.IdProducto = Productos.IdProducto WHERE Arriendos.ID=" & appID1
        Dim dt As DataTable = QueryAsDataTable(sql)
        'With DetalleDeArriendo
        '    .cbProductosArriendo.Text = ""
        '    .cbBloque.Text = ""
        '    .txtSearch.Text = ""
        '    .tbComentario.Text = ""
        '    .tbInfoExtra.Text = ""
        ...
        If dt.Rows.Count > 0 Then
            ...
            With DetalleDeArriendo
                ...
                .cbProductosArriendo.Text = row("NombreProducto")
    ...
    Private Sub AddAppointmentToflDay(ByVal startDayAtFlNumber As Integer)
        ...
        Dim sql As String = "SELECT Arriendos.ID,Arriendos.Fecha,Arriendos.IdProducto,Arriendos.Letrabloque,Arriendos.NombreCliente,Arriendos.Comentario FROM Arriendos WHERE (Arriendos.Fecha >= #" + startDate.ToString("dd-MM-yyyy") + "#) AND (Arriendos.fecha <= #" + endDate.ToString("dd-MM-yyyy") + "#)"
